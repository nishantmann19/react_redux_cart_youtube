...
function CustomInput(props) {
    ...
    const inputValues = () => {
        ...
        if (fieldType === "select" && dataList?.length !== 0) {
            return <select onChange={(event) => handleOnChange(fieldType, event)} {...rest}>
                {dataList?.map((data, index) => {
                    return <option key={`clientsList_${data?.id}_${index}`} value={data?.[valueKey]}>{multipleOptions?.length !== 0 ? multipleOptions?.map(options => data?.[options] + ', ') : data?.[optionsKey]}</option>
                })}
            </select>
        };
    };
